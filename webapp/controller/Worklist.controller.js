sap.ui.define([
	...
	"cmpc/model/formatter",
	"sap/ui/model/Filter",
	"sap/ui/model/FilterOperator",
	"cmpc/utils/VariantHelper",
	"cmpc/model/grouper",
	"cmpc/model/GroupSortState",
	"sap/m/GroupHeaderListItem",
	"cmpc/model/models",
	"cmpc/utils/VariantEjecHelper",
	"sap/ui/model/Sorter"
], function (BaseController, JSONModel, History, formatter, Filter, FilterOperator, VariantHelper, grouper, GroupSortState,
	GroupHeaderListItem,
	models, VariantEjecHelper, Sorter) {
	"use strict";
	var oThis;
	return BaseController.extend("cmpc.controller.Worklist", {
		formatter: formatter,
		...
		onInit: function () {
			...
			this.variantHelper.init(this.getView().byId("table"));
		...
		_onObjectMatched: function (oEvent) {
			...
			// hoja
			this.getView().byId("hoja").getBinding("items").filter(filtercod);

			//usuario  // true = AND, false = OR
			...
			//fecha
			var filterfecha = new sap.ui.model.Filter("fecha_crea", sap.ui.model.FilterOperator.EQ, codlib);
			this.getView().byId("fecha_crea").getBinding("items").filter(filterfecha);

			/*			//monto
						var filtermonto = new sap.ui.model.Filter("monto", sap.ui.model.FilterOperator.EQ, codlib);
						this.getView().byId("monto").getBinding("items").filter(filtermonto);*/

			//moneda
			var filtermoneda = new sap.ui.model.Filter("moneda", sap.ui.model.FilterOperator.EQ, codlib);
			this.getView().byId("moneda").getBinding("items").filter(filtermoneda);

			//PO N
			var filterpon = new sap.ui.model.Filter("po_n", sap.ui.model.FilterOperator.EQ, codlib);
			this.getView().byId("po_n").getBinding("items").filter(filterpon);

			//PO i
			var filterpoi = new sap.ui.model.Filter("po_i", sap.ui.model.FilterOperator.EQ, codlib);
			this.getView().byId("po_i").getBinding("items").filter(filterpoi);

			//cuenta
			var filtercuenta = new sap.ui.model.Filter("cuenta", sap.ui.model.FilterOperator.EQ, codlib);
			this.getView().byId("cuenta").getBinding("items").filter(filtercuenta);

			//Nombre
			var filternombre = new sap.ui.model.Filter("nombre", sap.ui.model.FilterOperator.EQ, codlib);
			this.getView().byId("nombre").getBinding("items").filter(filternombre);
			...
			var fdesc = new sap.ui.model.Filter("frgxt", "EQ", codlib);
			this.getView().byId("frgxt").getBinding("items").filter(fdesc);
		...
		onSearch: function (oEvent) {
			if (oEvent.getParameters().refreshButtonPressed) {
				...
				if (sQuery && sQuery.length > 0) {
					oTableSearchState = [
						new Filter("hoja", FilterOperator.Contains, sQuery),
						new Filter("usuario", FilterOperator.Contains, sQuery),
						new Filter("fecha_crea", FilterOperator.Contains, sQuery),
						new Filter("po_n", FilterOperator.Contains, sQuery),
						new Filter("po_i", FilterOperator.Contains, sQuery),
						new Filter("nombre", FilterOperator.Contains, sQuery),
						////new Filter("cod", FilterOperator.Contains, sQuery),
						new Filter("cuenta", FilterOperator.Contains, sQuery),
		...
		_showObject: function (oItem) {
			this.getRouter().navTo("object", {
				objectId: oItem.getBindingContext().getProperty("hoja")
			});
		},
		/**
		...
		/**
		 *@memberOf cmpc.controller.Worklist
		 */
		//inicio sort
		//*************************************************************************+
...
if (!sap.ui.getCore().byId("vsd")) {
		...
		*/
		liberar_sel: function () {
			...
			for (var i = 0; i < rowItems.length; i++) {
				if (rowItems[i].getSelected()) {
					var filterhoja = new sap.ui.model.Filter("hoja", sap.ui.model.FilterOperator.EQ, oModel.getProperty("hoja", rowItems[i].getBindingContext()));
		...
		onVariantChange: function (oEvent) {
			...
			this._Variant = sap.ui.xmlfragment("cmpc.view.Variant", this);
		...
		_applyFilter: function (aFilters) {
			//	debugger;
			if (this.getView().byId("hoja").getSelectedItems().length ||
				this.getView().byId("usuario").getSelectedItems().length ||
				this.getView().byId("fecha_crea").getSelectedItems().length ||
				//	this.getView().byId("monto").getSelectedItems().length ||
				this.getView().byId("moneda").getSelectedItems().length ||
				this.getView().byId("po_n").getSelectedItems().length ||
				this.getView().byId("po_i").getSelectedItems().length ||
				this.getView().byId("cuenta").getSelectedItems().length ||
				this.getView().byId("nombre").getSelectedItems().length ||
		...
		_BuildFilters: function (oList) {
			...
			//siempre debe llevar el c digo de liberaci n en el filtro que sea
			var oTable = this.byId("table"),
		...
		onGuardarVarianteEjec: function (oEvent) {
			...
			this._VariantEjec = sap.ui.xmlfragment("cmpc.view.VariantEjec", this);
		...
		onSaverVariantEjec: function (oEvent) {
			...
			if (!sap.ui.getCore().byId("NombreVarEjec").getValue() || !sap.ui.getCore().byId("TextVarEjec").getValue()) {
				...
				// Esta era la l gica original, se deja ahora que siempre sea una variante ( ejecuci n y variente)
		...
		onVariantesejecChange: function (oEvent) {
			...
			var aItems2 = this.getView().byId("hoja").getItems();
			var aFilters = [];
			for (var i3 = 0; i3 < aItems2.length; i3++) {

				aItems2[i3].setSelected(true);
				aFilters.push(new Filter("hoja", "EQ", aItems2[i3].getKey()));
			}
		...
		onFijarVarianteEjec: function () {
			...
			if (this.getView().byId("VariantesCEjec").getSelectedItem().getKey()) {
				...
				//	this.getView().getModel("VariantesEjec").create("/VarianteUsuarioSet", {
		...
		setInitialVariantEjec: function () {
			...
			var aItems2 = this.getView().byId("fecha_crea").getItems();

			for (var i3 = 0; i3 < aItems2.length; i3++) {

				aItems2[i3].setSelected(true);
				//aFilters.push(new Filter("Negocio", "EQ", aItems2[i3].getKey()));
				aFilters.push(new Filter("fecha_crea", "EQ", aItems2[i3].getKey()));
			}
				...
				columns: [{
					...
					template: {
						content: {
							...
							formatter: function (value) {
								if (value) {
									return this.formatter.moneda(value);
								} else {
									return "0";
								}
							}.bind(this)
		...
		obtenerInfoAnexos: function (oEvent) {
			...
			oThis.Anexodoc = "";
			oThis.Anexodoc = bindingContext.getProperty("hoja");
			Title = "Detalle de Anexos para HAS: " + oThis.Anexodoc;
			aFiltros.push(new sap.ui.model.Filter("Hoja", sap.ui.model.FilterOperator.EQ, oThis.Anexodoc));
		...
		onObtenerAnexo: function (oEvent) {
			...
			if (Anexo) {
				var oServicio = "/sap/opu/odata/sap/ZHAS_SERV_SRV/";
				if (Extension === "HTM" || Extension === "HTML") {
					lv_path = Anexo;
					window.open(lv_path, "_system");
				} else if (Extension === "TEXT") {
					if (!this.dialogNota) {
						this.dialogNota = sap.ui.xmlfragment("cmpc.view.Nota2", this);
